...
// ── WEBHOOKS (Google Ads, Meta Ads, Zapier/Make) ───────────
...
app.post('/api/leads/import', (req, res) => {
  const { leads: rows, skipDuplicates = true } = req.body;
  if (!Array.isArray(rows) || rows.length === 0) return err(res, 'No leads provided');
  if (rows.length > 5000) return err(res, 'Max 5000 leads per import');
  ...
  const checkDup = db.prepare('SELECT id FROM leads WHERE email = ? AND email != ""');
    ...
    for (const [i, row] of rows.entries()) {
      try {
        ...
        if (skipDuplicates && row.email) {
          const dup = checkDup.get(row.email.trim());
        ...
        insert.run({
          ...
          internal:     `Imported from Excel on ${new Date().toLocaleDateString('en-AU')}`,
        });
        imported++;
      } catch(e) {
